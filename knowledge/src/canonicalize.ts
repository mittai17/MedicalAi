...
type KeywordRow = [id: string, keywords: string[]];
...
export function canonicalizeSymptom(raw: string): string[] {
  ...
  for (const [id, keywords] of KEYWORD_MAP) {
    if (keywords.some((k) => s.includes(k))) hits.push(id);
  }

  // UI checklist exact names.
  if (s.includes("body aches")) hits.push("body_aches");
  if (s.includes("fatigue") || s.includes("weakness")) hits.push("fatigue");
  if (s.includes("skin rash") || s === "rash") hits.push("rash");
  if (s.includes("difficulty breathing")) hits.push("breathing_difficulty");
  if (s.includes("sore throat")) hits.push("sore_throat");
  if (s.includes("nausea") || s.includes("vomiting")) {
    if (s.includes("nausea")) hits.push("nausea");
    if (s.includes("vomiting")) hits.push("vomiting");
  }
