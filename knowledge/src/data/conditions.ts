...
export const CONDITION_BY_ID: ReadonlyMap<string, Condition> = new Map(
  CONDITIONS.map((c) => [c.id, c]),
);

/** Red-flag symptoms that escalate risk regardless of match score. */
export const RED_FLAG_SYMPTOMS: ReadonlySet<string> = new Set([
...
export function durationMultiplier(duration: string | undefined): number {
  switch (duration) {
...
export function durationRiskEscalation(duration: string | undefined): boolean {
