 ...
 */
export const DISEASE_ADVICE: Readonly<Record<string, DiseaseAdvice>> = {
  pneumonia: {
    condition: "Pneumonia",
 ...
 */
export function adviceFor(condition: string): DiseaseAdvice {
  const key = condition.trim().toLowerCase().replace(/_/g, " ");
  const direct: DiseaseAdvice | undefined =
    DISEASE_ADVICE[key] ?? DISEASE_ADVICE[key.replace(/ /g, "_")];
  if (direct) return direct;

  for (const [entryKey, advice] of Object.entries(DISEASE_ADVICE)) {
    const normalized = entryKey.replace(/_/g, " ");
    if (key.includes(normalized) || normalized.includes(key)) return advice;
  }
  return fallbackAdvice(condition);
}
 ...
 */
export function organAdvice(organ: string): DiseaseAdvice {
  return {
    condition: organ,
...
export function fallbackAdvice(condition: string): DiseaseAdvice {
  return {
    condition,
