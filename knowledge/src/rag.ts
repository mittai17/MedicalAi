...
export function normalizeTerms(text: string): Map<string, number> {
...
function buildIndex(documents: RagDocument[]): Indexed[] {
  return documents.map((document) => {
    ...
    return {
      ...
      seedTokens: Array.from(terms.entries()).flatMap(([term, count]) =>
 ...
 */
export function searchText(doc: RagDocument): string {
...
export class InMemoryRagRetriever implements RagRetriever {
  ...
  retrieve(query: string, limit = 3, category: string | null = null): RagDocument[] {
