 ...
 */
export function adviceFor(condition) {
    const key = condition.trim().toLowerCase().replace(/_/g, " ");
    const direct = DISEASE_ADVICE[key] ?? DISEASE_ADVICE[key.replace(/ /g, "_")];
    if (direct)
        return direct;
 ...
 */
export function organAdvice(organ) {
    return {
        condition: organ,
...
export function fallbackAdvice(condition) {
    return {
        condition,
