...
export function reason(params) {
    ...
    for (const condition of CONDITIONS) {
        ...
        if (precision > 0.05) {
            ...
            const f1 = precision + completeness > 0
...
export function assessVitalsRisk(vitals) {
    if (!vitals)
        return 0;
    let risk = 0;
 ...
 */
export function contextEscalations(evidence, vitals, patientContext) {
    if (!patientContext)
        return 0;
    let escalation = 0;
    ...
    const hasHypertension = conditions.includes("hypertension") || conditions.includes("high bp") || conditions.includes("high blood pressure");
    ...
    if (hasHypertension) {
        ...
        if ((!Number.isNaN(sys) && sys >= 160) || (!Number.isNaN(dia) && dia >= 100))
            escalation = 1;
    }
    if (hasDiabetes && (evidence["polyuria"] || evidence["irregular_sugar_level"]))
        escalation = 1;
    if (hasLungDisease && (evidence["breathing_difficulty"] || evidence["wheezing"]))
        escalation = 1;
    if (hasHeartDisease &&
...
export function computeRisk(condition, redFlag, vitalsRisk, prolonged, contextEscalation = 0) {
    const base = condition.baseRisk === "high" ? 3 : condition.baseRisk === "moderate" ? 2 : 1;
    let level = base;
    if (condition.redFlag || redFlag)
...
export function buildRecommendations(diagnosisId, condition, riskLevel, evidence, vitals, vitalsRisk) {
    const recs = [];
    let priority = 0;
...
export function noMatch(symptoms, vitals, diagnosisId, ragRetriever) {
    const vitalsRisk = assessVitalsRisk(vitals);
    const risk = vitalsRisk >= 2 ? "high" : vitalsRisk >= 1 ? "moderate" : "low";
    const recs = [];
    if (vitalsRisk >= 2) {
        recs.push({
            id: cryptoRandomId(),
    ...
    let advice = adviceFor("undifferentiated illness");
    if (ragRetriever) {
